...
export const BANNER_LINES = (harness, source, generator = DEFAULT_GENERATOR) => [
...
export function banner(format, { harness, source, generator = DEFAULT_GENERATOR }) {
  const lines = BANNER_LINES(harness, source, generator);
  if (format === "markdown") {
    return "<!--\n" + lines.map((l) => "  " + l).join("\n") + "\n-->\n";
  }
...
export function insertStamp(body, { format, harness, source, generator = DEFAULT_GENERATOR }) {
  ...
  const block = banner(format, { harness, source, generator }) + PROVENANCE_TOKEN + "\n";
...
export function stamp(body, { format, src, srcHash, pkg, project, harness, generator = DEFAULT_GENERATOR }) {
  const tokenized = insertStamp(body, { format, harness, source: src, generator });

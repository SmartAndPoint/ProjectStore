...
test("install spec acceptance: one emitter, one parser — no other script or hook carries the grammar", () => {
  for (const dir of ["scripts", "hooks"]) {
    for (const n of readdirSync(join(ROOT, dir)).filter((f) => f.endsWith(".mjs") && f !== "provenance.mjs")) {
      ...
      // Both the literal grammar and a regex source that would parse it (the
      // escaped `\d` a script would write). lib.mjs's refresh path recognises
      // our launcher by the template's header line, never by the grammar.
      assert.ok(!/projectstore: v(\d+|\\d\+?) src=/.test(src), `${dir}/${n} emits or parses the provenance grammar itself`);

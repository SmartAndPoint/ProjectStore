...
---
<!--
  GENERATED by scripts/build-adapters.mjs from commands/epic.md for the codex harness.
...
1. **Check config**: if `.codex/projectstore.json` is missing — instruct user to `/projectstore-bind` and stop.

2. **Validate args**: `$ARGUMENTS` must contain at least an ID and a title. ID is a short uppercase token (e.g. `AUTH-001`, `RECPLAT-269`). If only one word was given, ask user for the title via an approval prompt.
...
4. **Check collision**: if `<vault>/epics/<id>/epic.md` already exists, ask user via an approval prompt: "Epic `<id>` exists. [Open existing / Overwrite / Cancel]".
...
6. **Approval** via an approval prompt: Yes / Edit / No. This is the only gate: **Yes** covers the epic and its index row. Disclose in the question that the folder's whole managed index table is regenerated from vault state at write time, so the update may also repair a stale row for another epic.

7. **Pre-write race check** (Layer 1): run `test -e "<path>"`. The earlier collision check (step 4) covers most cases, but another session could have created this epic during the approval delay. If exists now → ask the user via an approval prompt whether to **Overwrite** or **Cancel**. Do not silently overwrite.

8. **On Yes** (path free or overwrite confirmed): write the file (parent directories are created by apply_patch), then create the stories directory: `mkdir -p "<vault>/epics/<id>/stories"`. The draft script itself never touches the disk — declining at step 6 leaves the vault unchanged.

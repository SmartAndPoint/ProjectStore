...
---
<!--
  GENERATED by scripts/build-adapters.mjs from commands/research.md for the codex harness.
...
2. Run `node "$PROJECTSTORE_ROOT/scripts/draft.mjs" research "$ARGUMENTS"`.
...
4. an approval prompt: Yes / Edit / No. This is the only gate: **Yes** covers the artifact and its index row. Disclose in the question that the folder's whole managed index table is regenerated from vault state at write time, so the update may also repair a stale row for another artifact.

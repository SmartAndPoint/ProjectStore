...
---
<!--
  GENERATED by scripts/build-adapters.mjs from commands/spec.md for the codex harness.
...
1. **Check config**: stop if `.codex/projectstore.json` missing.
...
4. **Preview**: path + full content, plus every `warnings` entry as a `⚠️`
   ...
   **topic collision** (`"<identity>" already exists as <with>`) and ask:
   ...
   an approval prompt: Yes / Edit / No. This is the only gate: **Yes** covers the
...
6. **On Yes**: write the file. Then, if `index` is non-null, apply its index
   row through the core — never the apply_patch tool, no second gate (the
...
7. **Reciprocal links**: for every entry in `stories:`, propose an edit to that
   ...
   `specs: ["SPEC-001"]`). One an approval prompt per file.
...
4. **Approval** via an approval prompt, then apply the edit (frontmatter lines

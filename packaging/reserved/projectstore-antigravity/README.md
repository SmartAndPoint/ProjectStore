...
Antigravity CLI is Google's terminal agent, and the successor to the retired Gemini CLI. Its plugin format has not been researched yet, so nothing is built against it — the name is held so it is available when something is.
